...
class LexPlugin(BasePlugin):
    ...
    def on_config(self, config):
        ...
        print(f"[mkdocs-lex] Downloading lexd binary...")
        asset_url, is_zip = self._get_latest_release_url()
        
        archive_path = os.path.join(cache_dir, 'lexd_archive' + ('.zip' if is_zip else '.tar.gz'))
        
        try:
            req = urllib.request.Request(asset_url, headers={'User-Agent': 'mkdocs-lex-plugin'})
            with urllib.request.urlopen(req) as response, open(archive_path, 'wb') as out_file:
                shutil.copyfileobj(response, out_file)
        except Exception as e:
            raise PluginError(f"[mkdocs-lex] Failed to download lexd from {asset_url}: {e}")
        ...
        try:
            ...
            raise PluginError(f"[mkdocs-lex] Failed to extract archive {archive_path}: {e}")
        ...
        if not os.path.exists(local_bin_path):
            ...
            for root, _, files in os.walk(cache_dir):
                if bin_name in files:
                    ...
                    break
            if not found:
                raise PluginError(f"[mkdocs-lex] Could not find '{bin_name}' inside the downloaded archive.")
        ...
        self.lexd_path = local_bin_path
        print(f"[mkdocs-lex] Successfully downloaded and extracted lexd to {local_bin_path}")
    ...
    def _get_latest_release_url(self):
        ...
        try:
            req = urllib.request.Request(api_url, headers={'User-Agent': 'mkdocs-lex-plugin'})
            with urllib.request.urlopen(req) as response:
                data = json.loads(response.read().decode('utf-8'))
        except Exception as e:
            raise PluginError(f"[mkdocs-lex] Failed to fetch latest release from GitHub API: {e}")
        ...
        raise PluginError(f"[mkdocs-lex] Could not find a matching release asset for {arch} {os_name}.")
